...
SkywayManager.defaultConstraints() を使うと、Room SDK で推奨するビデオ設定をプロジェクト内の他画面でも共有できます。SKWNavigator.initialize(_:) を呼ぶ前に Peer の破棄状態をチェックし、ストリームは必ず close() / SKWNavigator.terminate() でクリーンアップしてください。
