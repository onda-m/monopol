...
1. 画面のライフサイクルに合わせて `SkywayManager.shared.startSession(delegate:)` で `SKWPeer` を初期化する（未初期化のまま `prepareLocalStream` を呼ぶとエラー通知が返ります）。旧 SkyWay P2P コードから移行する場合は `SkywayManager.shared.ensurePeer(id:delegate:apiKey:domain:)` を使うと、既存の ID を維持したまま Room SDK 用のピアを共有できます。
...
SkywayManager.defaultConstraints() を使うと、Room SDK で推奨するビデオ設定をプロジェクト内の他画面でも共有できます。SKWNavigator.initialize(_:) を呼ぶ前に Peer の破棄状態をチェックし、ストリームは必ず close() / SKWNavigator.terminate() でクリーンアップしてください。旧コンポーネントが SkyWay オプションの組み立てやストリーム生成を直接行っていた箇所は、順次 `ensurePeer` と `prepareLocalStream` を経由させることで Room SDK 向けの初期化とクリーンアップが統一されます。
