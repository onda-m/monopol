...
extension MediaConnectionViewController{
    ...
    func closeMedia() {
        ...
        if let remoteVideoStream = roomSession.remoteVideoStream {
            remoteStreamView.detach(from: remoteVideoStream)
        }
    }
    ...
    func setup(){
            ...
            let roomName = makeRoomName(targetUserId: Int(liveCastId) ?? 0)
...
extension MediaConnectionViewController{
    private func configureRoomSessionCallbacks() {
        ...
        roomSession.onRemoteAudioStream = { _ in }
    ...
    @objc func audioSessionRouteChanged(_ notification: Notification) {
        ...
        roomSession.setRemoteAudioEnabled(false)
        
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
            // headphone
            self.roomSession.setRemoteAudioEnabled(true)
        }
    }
    ...
    private func handleRemoteMemberLeft() {
